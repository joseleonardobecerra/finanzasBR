const PresupuestosTab = ({ 
  ...
  const [nuevoVar, setNuevoVar] = useState({ 
    categoria: '', 
    limite: '' 
  });
  ...
  const [nuevoFijo, setNuevoFijo] = useState({ 
    descripcion: '', 
    monto: '', 
    categoria: categoriasMaestras[0] || 'Otros', 
    diaPago: '' 
  });
  ...
  // CÁLCULO SEGURO ANTI-ERRORES
  const fijosItems = useMemo(() => {
    return pagosFijos.map(pf => {
      // Prevención de NaN o fallos por null
      const descFijo = (pf.descripcion || '').toLowerCase();
      
      const ejecutado = egresosMes
        .filter(e => 
          e.tipo === 'Fijo' && 
          (e.descripcion || '').toLowerCase().includes(descFijo)
        )
    ...
    return (
      <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col relative overflow-hidden group hover:border-slate-600 transition-colors shadow-lg">
        ...
        <div className="flex justify-between text-[11px] text-slate-400 mb-1">
          <span>
            Límite Estimado: <span className="font-bold text-slate-300">{formatCOP(p.limite)}</span>
          </span>
        </div>
        ...
        <div className="w-full bg-slate-900 rounded-full h-1.5 mt-auto border border-slate-800/50">
          <div 
            className={`h-1.5 rounded-full ${finalBarColor} transition-all duration-1000`} 
      ...
      <Card className={`border-t-4 ${tipoForm === 'variable' ? 'border-t-blue-500' : 'border-t-amber-500'} transition-colors duration-300`}>
        ...
        {tipoForm === 'variable' ? (
          <form 
            ...
            className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-in slide-in-from-left-4 duration-300"
          >
            ...
            <div className="md:col-span-1 flex items-end">
              <div className="flex gap-2 w-full">
                <button 
                  type="submit" 
                  className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95"
                >
                ...
                {editId && (
                  <button 
                    type="button" 
                    onClick={cancelEdit} 
                    className="bg-slate-800 hover:bg-slate-700 text-white py-2 px-3 rounded-xl transition-colors"
                  >
                    Cancelar
                  </button>
                )}
          ...
          /* MODO: FIJO */
          <form 
            onSubmit={handleAddFijo} 
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 animate-in slide-in-from-right-4 duration-300"
          >
            <div className="md:col-span-2">
              <label className="text-xs font-bold text-slate-500 uppercase">
                Nombre del Pago
            ...
            <div className="md:col-span-1 flex items-end">
               <div className="flex gap-2 w-full">
                  <button 
                    type="submit" 
                    className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold py-2 px-4 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95"
                  >
                    {editId ? <CheckSquareIcon/> : <PlusIcon/>}
                    {editId ? 'Guardar' : 'Agregar'}
                  </button>
        ...
        {(filtroLista === 'Todos' || filtroLista === 'Variables') && (
          <div className="space-y-4 animate-in fade-in">
            ...
            {varItems.length > 0 ? (
              ...
              <p className="text-sm text-slate-500 italic">
                No hay límites variables configurados.
              </p>
            )}
          </div>
        )}
