const SettingsTab = (() => {
  // ============================================================================
  // COMPONENTES UI EXTERNOS (Privados para Settings)
  // ============================================================================
  const Settings2 = ({ size = 24, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>;
  const FileSpreadsheet = ({ size = 16, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><path d="M8 13h2"/><path d="M8 17h2"/><path d="M14 13h2"/><path d="M14 17h2"/></svg>;
  ...
  const Upload = ({ size = 20, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>;
  const CodeIcon = ({ size = 16, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="m18 16 4-4-4-4"/><path d="m6 8-4 4 4 4"/><path d="m14.5 4-5 16"/></svg>;
  ...
  // ============================================================================
  const SettingsTabComponent = ({ stateData, importAllState, selectedMonth, showToast }) => {
    const { useRef } = React;
    ...
    const handleExportJSON = () => { 
      ...
      downloadAnchorNode.setAttribute("download", `respaldo_finanzas_${new Date().toISOString().split('T')[0]}.json`); 
    ...
    const handleImportJSON = (e) => { 
      ...
      reader.onload = (event) => { 
        try { 
          ...
          importAllState(parsed).then(() => showToast("¡Respaldo JSON restaurado con éxito!")).catch(() => showToast("Error al restaurar.", "error")); 
        } catch (err) { 
          showToast("Error al leer JSON.", "error"); 
        } 
      }; 
    ...
    return (
      <div className="max-w-4xl mx-auto animate-in fade-in duration-500 space-y-6 pb-20 md:pb-0">
        <header>
          <h1 className="text-2xl md:text-3xl font-black text-white tracking-wide flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#111222] shadow-neumorph-inset flex items-center justify-center border border-white/[0.05]">
               <Settings2 className="text-[#8A92A6]" /> 
            </div>
            Sistema y Ajustes
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <CardUI className="!border-transparent flex flex-col justify-between">
            <div className="flex gap-4 items-center mb-6">
              <div className="w-12 h-12 bg-[#111222] shadow-neumorph-inset text-emerald-400 rounded-xl flex items-center justify-center border border-transparent hover:border-emerald-500/30 transition-colors shrink-0">
                <Download className="drop-shadow-[0_0_5px_rgba(52,211,153,0.4)]"/>
              </div>
              <div>
          ...
          <CardUI className="!border-transparent flex flex-col justify-between">
            <div className="flex gap-4 items-center mb-6">
              <div className="w-12 h-12 bg-[#111222] shadow-neumorph-inset text-emerald-400 rounded-xl flex items-center justify-center border border-transparent hover:border-emerald-500/30 transition-colors shrink-0">
                <Upload className="drop-shadow-[0_0_5px_rgba(52,211,153,0.4)]"/>
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <CardUI className="!border-transparent">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-10 h-10 bg-[#111222] shadow-neumorph-inset text-neoncyan rounded-xl flex items-center justify-center shrink-0 border border-neoncyan/20">
                <Download className="drop-shadow-[0_0_5px_rgba(0,229,255,0.4)]"/>
              </div>
          ...
          <CardUI className="!border-transparent">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-10 h-10 bg-[#111222] shadow-neumorph-inset text-neonmagenta rounded-xl flex items-center justify-center shrink-0 border border-neonmagenta/20">
                <Upload className="drop-shadow-[0_0_5px_rgba(255,0,122,0.4)]"/>
              </div>
