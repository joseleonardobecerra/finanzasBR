...
const Plus = ({ size = 16, strokeWidth="3", className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className={className}><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>;
...
const PiggyBank = ({ size = 16, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M19 5c-1.5 0-2.8 1.4-3 2-3.5-1.5-11-.3-11 5 0 1.8 0 3 2 4.5V20h4v-2h3v2h4v-4c1-.5 1.5-1 2-1.5L22 12c0-4-3-7-3-7z"/><path d="M2 9v1c0 1.1.9 2 2 2h1"/><path d="M16 11h.01"/></svg>;
const TrendingUp = ({ size = 16, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>;
...
const InputUI = ({ type="text", label, value, onChange, className, placeholder, step, required }) => (
  <div className={`relative ${className?.includes('col-span') ? className : 'w-full'}`}>
    ...
    <input type={type} step={step} value={value} onChange={onChange} placeholder={placeholder} required={required} className={`w-full bg-[#111222] shadow-neumorph-inset border border-transparent rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-emerald-500 transition-all duration-300 placeholder:text-slate-600 ${className}`} />
  </div>
);

// ✨ COMPONENTE: Logos de Bancos Dinámicos
...
const InversionesTab = ({ cuentas, addCuenta, updateCuenta, removeCuenta,
  ...
  const guardarGanancia = (c) => {
    ...
    // Validamos si estamos viendo el mes actual o un mes histórico
    const fechaActual = getLocalToday(); // Ej: "2026-04-15"
    const isCurrentMonth = selectedMonth === fechaActual.slice(0, 7);
    
    // Si estamos en el mes actual, usa el día de hoy. Si es un mes histórico, usa el 28 por defecto.
        ...
        <Card className="!border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.2)] mb-4 animate-in slide-in-from-top-4">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-2 md:grid-cols-4 gap-5 items-end">
            <div className="col-span-2">
              <InputUI required label="Nombre de la Cuenta" value={nuevo.name} onChange={e=>setNuevo({...nuevo, name: e.target.value})} placeholder="Ej. Ficción, CDT..." />
            </div>
            
            <div className="col-span-1 relative">
              <InputUI type="number" label="Saldo Base/Inicial ($)" value={nuevo.initialBalance} onChange={e=>setNuevo({...nuevo, initialBalance: e.target.value})} className="pl-8" placeholder="0" />
              <span className="absolute left-4 top-[38px] text-base font-black text-slate-600">$</span>
            </div>
            
            <div className="col-span-1 relative">
              <InputUI type="number" step="0.01" label="Tasa E.A. (%)" value={nuevo.tasaEA} onChange={e=>setNuevo({...nuevo, tasaEA: e.target.value})} className="pr-8 font-bold text-amber-400" placeholder="0.0" />
