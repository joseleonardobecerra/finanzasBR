  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 md:pb-0">
      ...
      <Card className="border-t-4 border-t-rose-500 transition-all duration-300">
        ...
        >
          <h2 className="text-lg font-bold text-white flex items-center gap-2">1. Registrar Gasto Individual</h2>
        ...
        {openSections.form && (
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-6 gap-4 mt-4 animate-in slide-in-from-top-4 fade-in duration-300">
            ...
            <div className="mt-4 flex-1 overflow-y-auto max-h-[350px] pr-1 space-y-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-700 [&::-webkit-scrollbar-thumb]:rounded-full animate-in slide-in-from-top-4 fade-in duration-300">
              {pagosFijosOrdenados.length === 0 ? (
                ...
                pagosFijosOrdenados.map(pf => {
                  ...
                  return (
                    <div key={pf.id} className={`p-3 rounded-xl border flex flex-col md:flex-row md:items-center justify-between transition-all gap-3 ${isPaid ? 'bg-emerald-900/10 border-emerald-500/20 opacity-60' : 'bg-slate-950 border-slate-800 hover:border-orange-500/30'}`}>
                      
                      {/* Lado Izquierdo: Botón de check y Título */}
                      ...
                      {/* Lado Derecho: Controles de edición o Valor final */}
                      {!isPaid ? (
