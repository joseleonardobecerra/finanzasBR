...
function App() {
  ...
  // Íconos (mantenidos localmente porque se usan en el wizard de App.js)
  const XIcon = ({ size=24 }) => (
  ...
  // FIX #1: loadedRef se resetea a 0 cada vez que authUser cambia (nuevo login),
  // evitando la condición de carrera entre sesiones.
  ...
  useEffect(() => {
    if (!authUser) return;
    // Reset del contador y del estado de carga al inicio de cada sesión
    loadedRef.current = 0;
    setAppCargando(true);
  ...
  // FIX #2: Reemplazamos los "magic strings" de primas por un campo de modelo mesEspecifico.
  // Para compatibilidad con datos existentes (que usan el texto), se mantiene el fallback.
  const activeIngresosFijos = useMemo(() => {
    ...
    return ingresosFijos.filter(inf => {
      ...
      let passMonth = true;
      // Método nuevo: campo mesEspecifico (número de mes '01'-'12')
      if (inf.mesEspecifico) {
        passMonth = inf.mesEspecifico === currentMonthNum;
      } else {
        // Fallback para compatibilidad con registros antiguos que usan texto
  ...
  // FIX #3: egresosMes dentro de useMemo para evitar recálculo en cada render
        ...
        <div className="p-4 md:p-8 overflow-y-auto flex-1 relative">
          <div className="max-w-6xl mx-auto">
            <ErrorBoundary>
              ...
              {/* FIX #4: AnaliticaTab ahora recibe datos FILTRADOS (activeIngresos/activeEgresos)
                  en lugar de los datos brutos (ingresos/egresos), para que el filtro por persona
