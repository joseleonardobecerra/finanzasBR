...
function App() {
  ...
  const TOTAL_COL = 8; // Reducido a 8 tras limpiar colecciones innecesarias
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      col('egresos', setEgresos), col('presupuestos', setPresupuestos),
  ...
  const importAllState = async (p) => {
    const cols = [['cuentas', p.cuentas], ['transferencias', p.transferencias], ['ingresos', p.ingresos], ['egresos', p.egresos], ['presupuestos', p.presupuestos], ['pagosFijos', p.pagosFijos], ['ingresosFijos', p.ingresosFijos]];
  ...
  const activeIngresosFijos = useMemo(() => {
    ...
    return ingresosFijos.filter(inf => {
      ...
      let passMonth = true;
      if (inf.mesEspecifico) { passMonth = inf.mesEspecifico === currentMonthNum; } else {
        ...
        <div className="p-4 md:p-8 overflow-y-auto flex-1 relative">
          <div className="max-w-6xl mx-auto">
            <ErrorBoundary>
              ...
              {activeTab === 'settings' && <SettingsTab stateData={{cuentas, transferencias, ingresos, egresos, presupuestos, pagosFijos, ingresosFijos}} importAllState={importAllState} selectedMonth={selectedMonth} showToast={showToast} />}
            </ErrorBoundary>
          </div>
        </div>
      </main>
          ...
          <div className="bg-[#17171a] w-full md:w-[400px] md:rounded-3xl rounded-t-3xl p-6 border border-slate-800 shadow-2xl animate-in slide-in-from-bottom-10 min-h-[400px] flex flex-col relative">
            <div className="flex justify-between items-center mb-6 shrink-0">
               <div className="flex items-center gap-3">
                 ...
                 <div><h3 className="text-lg font-black text-white tracking-tight">Registro Rápido</h3><div className="flex gap-1 mt-1.5">{[1,2,3,4,5].map(s => (<div key={s} className={`h-1 w-[18px] rounded-full transition-all duration-500 ${s <= qeStep ? 'bg-indigo-500' : 'bg-slate-800'}`}></div>))}</div></div>
               </div>
            ...
            <div className="flex-1 flex flex-col justify-center animate-in fade-in slide-in-from-right-4 duration-300">
              {qeStep === 1 && (<div className="space-y-4"><h4 className="text-center text-slate-400 font-bold mb-6">¿Qué vas a registrar?</h4><button onClick={() => { setQeType('egreso'); setQeStep(2); }} className="w-full flex items-center justify-center gap-3 p-6 bg-rose-500/10 border-2 border-rose-500/30 hover:border-rose-500 rounded-2xl text-rose-400 font-black text-xl transition-all hover:scale-[1.02] shadow-lg shadow-rose-500/5">📉 Es un Gasto</button><button onClick={() => { setQeType('ingreso'); setQeStep(2); }} className="w-full flex items-center justify-center gap-3 p-6 bg-emerald-500/10 border-2 border-emerald-500/30 hover:border-emerald-500 rounded-2xl text-emerald-400 font-black text-xl transition-all hover:scale-[1.02] shadow-lg shadow-emerald-500/5">📈 Es un Ingreso</button></div>)}
              {qeStep === 2 && (<div className="space-y-6"><div><label className={`text-xs font-bold uppercase tracking-wider block mb-2 ${qeType === 'egreso' ? 'text-rose-500' : 'text-emerald-500'}`}>Monto exacto</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl font-black text-slate-500">$</span><input type="number" value={qeMonto} onChange={(e)=>setQeMonto(e.target.value)} className="w-full bg-slate-950 border-2 border-slate-800 focus:border-indigo-500 text-white rounded-2xl pl-10 pr-4 py-4 text-3xl font-black outline-none transition-colors shadow-inner" placeholder="0" autoFocus /></div></div><div><label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Descripción corta (Opcional)</label><input type="text" value={qeDescripcion} onChange={(e)=>setQeDescripcion(e.target.value)} className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 text-white rounded-xl px-4 py-3 text-sm font-medium outline-none transition-colors" placeholder="Ej. Almuerzo KFC" /></div><button disabled={!qeMonto} onClick={() => setQeStep(3)} className="w-full py-4 rounded-xl font-black text-white text-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:bg-slate-800 transition-all shadow-lg mt-2">Siguiente</button></div>)}
              {qeStep === 3 && (<div className="h-full flex flex-col"><h4 className="text-center text-slate-400 font-bold mb-4">Selecciona la categoría</h4><div className="flex-1 overflow-y-auto pr-2 pb-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-700 [&::-webkit-scrollbar-thumb]:rounded-full"><div className="grid grid-cols-2 gap-2">{(qeType === 'egreso' ? categoriasMaestras : ['Salario', 'Honorarios', 'Transferencia', 'Inversión', 'Regalo', 'Otros']).map(cat => (<button key={cat} onClick={() => { setQeCategoria(cat); setQeStep(4); }} className={`p-3 rounded-xl text-xs font-bold text-left transition-all border bg-slate-900 border-slate-800 text-slate-300 hover:border-indigo-500 hover:bg-indigo-500/10 active:scale-95`}>{cat}</button>))}</div></div></div>)}
              {qeStep === 4 && (<div className="space-y-4"><h4 className="text-center text-slate-400 font-bold mb-6">{qeType === 'egreso' ? '¿Cómo lo pagaste?' : '¿A dónde entró el dinero?'}</h4><button onClick={() => { setQeMethod('cash'); setQeStep(5); }} className="w-full p-4 rounded-xl text-sm font-bold text-left transition-all border flex items-center gap-3 bg-slate-900 border-slate-800 text-slate-300 hover:border-indigo-500 hover:bg-indigo-500/10 active:scale-95"><span className="text-xl">💵</span> Efectivo</button><button onClick={() => { setQeMethod('bank'); setQeStep(5); }} className="w-full p-4 rounded-xl text-sm font-bold text-left transition-all border flex items-center gap-3 bg-slate-900 border-slate-800 text-slate-300 hover:border-indigo-500 hover:bg-indigo-500/10 active:scale-95"><span className="text-xl">🏦</span> Cuenta Débito / Ahorros</button>{qeType === 'egreso' && (<button onClick={() => { setQeMethod('credit'); setQeStep(5); }} className="w-full p-4 rounded-xl text-sm font-bold text-left transition-all border flex items-center gap-3 bg-slate-900 border-slate-800 text-slate-300 hover:border-indigo-500 hover:bg-indigo-500/10 active:scale-95"><span className="text-xl">💳</span> Tarjeta de Crédito</button>)}</div>)}
              {qeStep === 5 && (<div className="space-y-6"><h4 className="text-center text-slate-400 font-bold mb-2">{qeType === 'egreso' ? '¿De cuál cuenta exactamente?' : '¿A qué cuenta exactamente?'}</h4><div className="grid grid-cols-1 gap-2 overflow-y-auto max-h-[250px] pr-1">{activeCalculatedAccounts.filter(c => c.type === qeMethod).map(acc => (<button key={acc.id} onClick={() => setQeCuenta(acc.id)} className={`p-4 rounded-xl text-sm font-bold text-left transition-all border flex justify-between items-center ${qeCuenta === acc.id ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-600'}`}><span className="flex items-center gap-2">{acc.name}</span>{qeCuenta === acc.id && <div className="w-2 h-2 bg-white rounded-full"></div>}</button>))}{activeCalculatedAccounts.filter(c => c.type === qeMethod).length === 0 && (<div className="text-center p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl mt-2"><p className="text-rose-400 text-xs font-bold">No tienes cuentas de este tipo registradas.</p></div>)}</div>{qeCuenta && (<button onClick={handleQuickSave} className={`w-full py-4 rounded-xl font-black text-white text-lg transition-transform hover:scale-[1.02] animate-in fade-in slide-in-from-bottom-4 shadow-xl ${qeType === 'egreso' ? 'bg-rose-600 shadow-rose-500/20' : 'bg-emerald-600 shadow-emerald-500/20'}`}>¡Guardar Definitivo!</button>)}</div>)}
            </div>
          </div>
        </div>
      )}
