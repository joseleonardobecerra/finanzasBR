...
const getTasaMensual = (tasaEA) => Math.pow(1 + (Number(tasaEA) || 0) / 100, 1 / 12) - 1;
...
const App = (() => {
  ...
  // ÍCONOS SVG
  // ============================================================================
  const SvgWrapper = ({ size = 18, className = "", children, strokeWidth = 2 }) => (
    <svg
      width={size}
      height={size}
      className={className}
  ...
  class ErrorBoundary extends React.Component {
    ...
    componentDidCatch(error, errorInfo) {
      console.error("Error capturado por la Cápsula:", error, errorInfo);
    }
      ...
      if (this.state.hasError) {
        return (
          <div className="p-6 bg-[#111222] shadow-neumorph-inset border-l-4 border-neonmagenta rounded-3xl m-4 md:m-8 text-white relative z-50">
            <h2 className="font-black text-xl mb-3 flex items-center gap-2 text-neonmagenta">
              <AlertCircle size={24} /> Sistema Protegido
            </h2>

            <p className="text-sm font-bold opacity-80 mb-6">
              El motor interceptó un error de renderizado y evitó que la aplicación colapsara.
            </p>
            ...
            <button
              ...
              className="px-6 py-3 bg-neonmagenta hover:bg-[#ff1a8c] shadow-glow-magenta text-[#0b0c16] rounded-xl text-sm font-black transition-all tracking-widest uppercase"
            >
              Recargar Vista
            </button>
          </div>
  ...
  const AppComponent = () => {
    ...
    const markLoaded = () => {
      loadedRef.current += 1;
      if (loadedRef.current >= TOTAL_COL) setAppCargando(false);
    };
    ...
    const changeMonth = (offset) => {
      ...
      const d = new Date(y, parseInt(m, 10) - 1 + offset, 1);
    ...
    const getMonthName = (ym) => {
      ...
      return new Date(y, parseInt(m, 10) - 1, 1).toLocaleString('es-ES', {
        month: 'long',
        year: 'numeric'
      });
    };
    ...
    useEffect(() => {
      ...
      const sanitizeData = (item) => {
        ...
        for (let key in clean) {
          if (clean[key] && typeof clean[key] === 'object') {
            if (clean[key].específico) {
              clean[key] = clean[key].específico;
            } else if (clean[key].especifico) {
              clean[key] = clean[key].especifico;
            } else if (clean[key].sub) {
              clean[key] = clean[key].sub;
            }
          }
        }
      ...
      const col = (name, setter) => db.collection(name).onSnapshot(
        snap => {
      ...
      const unsubs = [
        ...
        db.collection('sistema').doc('scoreHistory').onSnapshot(
          snap => {
            if (snap.exists) setScoreHistory(snap.data() || {});
            markLoaded();
          },
          err => {
            console.error('Error leyendo scoreHistory:', err);
            markLoaded();
          }
        )
      ];
    ...
    const fire = {
      add: (colName, data) => {
        ...
        return db.collection(colName).doc(safeData.id).set(safeData);
      },

      update: (colName, id, data) => {
        if (!id) return Promise.resolve();
        return db.collection(colName).doc(id).set(data, { merge: true });
      },
      ...
      bulkReplace: async (colName, newArray = []) => {
        const cleanArray = Array.isArray(newArray)
          ? newArray.map(item => ({
              ...item,
        ...
        for (let i = 0; i < cleanArray.length; i += 450) {
          const batch = db.batch();

          cleanArray.slice(i, i + 450).forEach(item => {
            batch.set(db.collection(colName).doc(item.id), item);
          });
    ...
    const importAllState = async (p = {}) => {
      const cols = [
        ...
        ['pagosFijos', p.pagosFijos],
        ['comprasCuotas', p.comprasCuotas],
        ['ingresosFijos', p.ingresosFijos]
      ];

      for (const [n, a] of cols) {
        if (Array.isArray(a)) await fire.bulkReplace(n, a);
      }
    };

    const addPagoFijoToState = (pf) => addPagoFijo({
      ...pf,
      id: generateId(),
    ...
    const categoriasMaestras = useMemo(() => Object.keys(CATEGORIAS_CONFIG), []);
    ...
    const calculatedAccounts = useMemo(() => {
      ...
      cuentas.forEach(c => {
        ...
        accMap[c.id] = {
          ...c,
          currentBalance: safeNumber(c.initialBalance),
          currentDebt: safeNumber(c.initialDebt),
          montoPrestado: safeNumber(c.montoPrestado) || safeNumber(c.initialDebt),
          totalPagado: safeNumber(c.totalPagadoPrevio),
          lastPaymentDate: c.lastPaymentDate || null
        };
      });

      ingresos.forEach(i => {
        if (i && i.cuentaId && accMap[i.cuentaId]) {
          accMap[i.cuentaId].currentBalance += safeNumber(i.monto);
        }
      });
      ...
      sortedEgresos.forEach(e => {
        ...
        const monto = safeNumber(e.monto);

        if (e.cuentaId && accMap[e.cuentaId]) {
          if (['credit', 'loan'].includes(accMap[e.cuentaId].type)) {
            accMap[e.cuentaId].currentDebt += monto;
          } else {
            accMap[e.cuentaId].currentBalance -= monto;
          }
        }

        if (e.deudaId && accMap[e.deudaId]) {
          const account = accMap[e.deudaId];
          account.totalPagado += monto;

          if (account.type === 'loan') {
            let capital = monto;
            const tasaEA = safeNumber(account.tasaEA);

            if (account.lastPaymentDate && e.fecha) {
              const diffMs = Math.abs(new Date(e.fecha) - new Date(account.lastPaymentDate));
              const diffDays = Number.isFinite(diffMs) ? Math.round(diffMs / 86400000) : 0;
              const tasaDiaria = Math.pow(1 + tasaEA / 100, 1 / 360) - 1;
              capital = monto - account.currentDebt * tasaDiaria * diffDays;
            } else {
              capital = monto - account.currentDebt * getTasaMensual(tasaEA);
            }

            account.lastPaymentDate = e.fecha || account.lastPaymentDate;
            account.currentDebt = Math.max(0, account.currentDebt - capital);
          } else {
            account.currentDebt = Math.max(0, account.currentDebt - monto);
          }
        }
      });
      ...
      transferencias.forEach(t => {
        ...
        if (t.fromId && accMap[t.fromId]) {
          if (['credit', 'loan'].includes(accMap[t.fromId].type)) {
            accMap[t.fromId].currentDebt += monto;
          } else {
            accMap[t.fromId].currentBalance -= monto;
          }
        }

        if (t.toId && accMap[t.toId]) {
          if (['credit', 'loan'].includes(accMap[t.toId].type)) {
            accMap[t.toId].currentDebt = Math.max(0, accMap[t.toId].currentDebt - monto);
          } else {
            accMap[t.toId].currentBalance += monto;
          }
        }
      });
    ...
    // ============================================================================
    // FILTRO POR PERSONA SOLO PARA DEUDAS
    // ============================================================================
    const getOwnerFallback = (textValue) => {
      if (!textValue) return 'Shared';

      const t = String(textValue).toUpperCase();
    ...
    const personaFilterEnabled = activeTab === 'deudas';

    const belongsToFilter = (owner) => {
      return !personaFilterEnabled ||
        filtroPersona === 'Total' ||
        owner === 'Shared' ||
        owner === filtroPersona;
    };
    ...
    const activeCalculatedAccounts = useMemo(() => {
      return calculatedAccounts.filter(c => {
        if (!c) return false;
        return belongsToFilter(c.ownerId || getOwnerFallback(c.name));
      });
    }, [calculatedAccounts, filtroPersona, activeTab]);

    const activeIngresos = useMemo(() => {
      return ingresos.filter(i => {
        ...
        return belongsToFilter(
          accOwner !== 'Shared'
            ? accOwner
            : (i.ownerId || getOwnerFallback(`${safeText(i.persona)} ${safeText(i.descripcion)}`))
        );
      });
    }, [ingresos, cuentas, filtroPersona, activeTab]);

    const activeEgresos = useMemo(() => {
      return egresos.filter(e => {
        ...
        return belongsToFilter(
          accOwner !== 'Shared'
            ? accOwner
            : (e.ownerId || getOwnerFallback(`${safeText(e.descripcion)} ${safeText(e.categoria)}`))
        );
      });
    }, [egresos, cuentas, filtroPersona, activeTab]);

    const activePagosFijos = useMemo(() => {
      return pagosFijos.filter(pf => {
        if (!pf) return false;
        return belongsToFilter(pf.ownerId || getOwnerFallback(`${safeText(pf.descripcion)} ${safeText(pf.categoria)}`));
      });
    }, [pagosFijos, filtroPersona, activeTab]);
    ...
    const activeIngresosFijos = useMemo(() => {
      ...
      return ingresosFijos.filter(inf => {
        ...
        const cuentaDestino = cuentas.find(c => c.id === inf.cuentaId);

        const owner = inf.ownerId || (
          cuentaDestino
            ? (cuentaDestino.ownerId || getOwnerFallback(cuentaDestino.name))
            : getOwnerFallback(inf.descripcion)
        );

        const passFilter = belongsToFilter(owner);

        let passMonth = true;

        if (inf.mesEspecifico) {
          passMonth = inf.mesEspecifico === selectedMonth || inf.mesEspecifico === currentMonthNum;
        } else {
          const descLower = String(inf.descripcion || '').toLowerCase();

          if (descLower.includes('prima 1')) {
            passMonth = currentMonthNum === '07';
          } else if (descLower.includes('prima 2')) {
            passMonth = currentMonthNum === '12';
          }
        }

        return passFilter && passMonth;
      });
    }, [ingresosFijos, cuentas, filtroPersona, selectedMonth, activeTab]);

    const activePresupuestos = useMemo(() => {
      return presupuestos.filter(p => {
        if (!p) return false;
        return belongsToFilter(p.ownerId || getOwnerFallback(p.categoria));
      });
    }, [presupuestos, filtroPersona, activeTab]);

    const activeComprasCuotas = useMemo(() => {
      return comprasCuotas.filter(c => {
        if (!c) return false;
        ...
        return belongsToFilter(
          accOwner !== 'Shared'
            ? accOwner
            : (c.ownerId || getOwnerFallback(c.descripcion))
        );
      });
    }, [comprasCuotas, cuentas, filtroPersona, activeTab]);

    const activeTransferencias = useMemo(() => {
      return transferencias.filter(t => {
        if (!t) return false;

        const ownerFrom = cuentas.find(c => c.id === t.fromId);
        const ownerTo = cuentas.find(c => c.id === t.toId);

        return belongsToFilter(ownerFrom ? (ownerFrom.ownerId || getOwnerFallback(ownerFrom.name)) : 'Shared') ||
          belongsToFilter(ownerTo ? (ownerTo.ownerId || getOwnerFallback(ownerTo.name)) : 'Shared');
      });
    }, [transferencias, cuentas, filtroPersona, activeTab]);

    // ============================================================================
    // INDICADORES MENSUALES
    // ============================================================================
    const isThisMonth = (f) => String(f || '').startsWith(selectedMonth);

    const ingresosMesTotal = useMemo(() => {
      return activeIngresos
        .filter(i => isThisMonth(i.fecha))
        .reduce((s, i) => s + safeNumber(i.monto), 0);
    }, [activeIngresos, selectedMonth]);

    const egresosMesTotal = useMemo(() => {
      return activeEgresos
        .filter(e => isThisMonth(e.fecha))
        .reduce((s, e) => s + safeNumber(e.monto), 0);
    }, [activeEgresos, selectedMonth]);

    const egresosMes = useMemo(() => {
      return activeEgresos.filter(e => isThisMonth(e.fecha));
    }, [activeEgresos, selectedMonth]);

    const cuotasMesTotal = useMemo(() => {
      return activeCalculatedAccounts
        .filter(c => ['credit', 'loan'].includes(c.type) && safeNumber(c.currentDebt) > 0)
        .reduce((s, c) => s + safeNumber(c.cuotaMinima), 0);
    }, [activeCalculatedAccounts]);

    const pagosDeCuotasEsteMes = useMemo(() => {
      return egresosMes
        .filter(e => {
          if (!e || e.tipo !== 'Fijo') return false;

          const categoria = String(e.categoria || '').toLowerCase();

          return categoria.includes('tarjeta') ||
            categoria.includes('crédito') ||
            categoria.includes('credito') ||
            categoria.includes('vehículo') ||
            categoria.includes('vehiculo') ||
            categoria.includes('davibank');
        })
        .reduce((sum, e) => sum + safeNumber(e.monto), 0);
    }, [egresosMes]);
    ...
    // ============================================================================
    // LIQUIDEZ VS INVERSIÓN / AHORRO SEPARADO
    // ============================================================================
    const isCuentaLiquidez = (c) => c && ['bank', 'cash'].includes(c.type);
    const isCuentaInversion = (c) => c && ['investment', 'pocket'].includes(c.type);

    const liquidezTotal = useMemo(() => {
      return activeCalculatedAccounts
        .filter(isCuentaLiquidez)
        .reduce((s, c) => s + safeNumber(c.currentBalance), 0);
    }, [activeCalculatedAccounts]);

    const inversionTotal = useMemo(() => {
      return activeCalculatedAccounts
        .filter(isCuentaInversion)
        .reduce((s, c) => s + safeNumber(c.currentBalance), 0);
    }, [activeCalculatedAccounts]);
    ...
    const deudaTotal = useMemo(() => {
      return activeCalculatedAccounts
        ...
        .reduce((s, c) => s + safeNumber(c.currentDebt), 0);
    }, [activeCalculatedAccounts]);

    const patrimonioDisponible = liquidezTotal + inversionTotal;

    const proyeccionLiquidez = useMemo(() => {
      const isPagoFijoRealizadoMes = (pf) => {
        return egresosMes.some(e => {
          if (!e || e.tipo !== 'Fijo') return false;
          if (e.pagoFijoId) return e.pagoFijoId === pf.id;

          return String(e.descripcion || '').toLowerCase() === String(pf.descripcion || '').toLowerCase();
        });
      };

      const pendientesEste = activePagosFijos
        .filter(pf => !isPagoFijoRealizadoMes(pf))
        .reduce((s, pf) => s + safeNumber(pf.monto), 0);

      const ingresosFijosLiquidezTotal = activeIngresosFijos
        .filter(inf => {
          const cuentaDestino = activeCalculatedAccounts.find(c => c.id === inf.cuentaId);
          return isCuentaLiquidez(cuentaDestino);
        })
        .reduce((s, inf) => s + safeNumber(inf.monto), 0);

      const totalFijosProx = activePagosFijos
        .reduce((s, pf) => s + safeNumber(pf.monto), 0);

      const liq30 = liquidezTotal + ingresosFijosLiquidezTotal - pendientesEste - cuotasMesRestantes;
      const liq60 = liq30 + ingresosFijosLiquidezTotal - totalFijosProx - cuotasMesTotal;
      const liq90 = liq60 + ingresosFijosLiquidezTotal - totalFijosProx - cuotasMesTotal;
    ...
    const scoreData = useMemo(() => {
      if (
        ...
        return {
          score: 0,
          desglose: [{ text: 'Aún no hay datos.', pts: 0, type: 'neutral' }],
          recs: [{ ico: '📝', title: 'Empieza tu registro', txt: 'Añade datos para evaluar tu salud.' }]
        };
      }
      ...
      if (flujoNetoMes < 0) {
        scr -= 40;
        desgloseArr.push({ text: 'Flujo negativo', pts: -40, type: 'danger' });
        rr.push({ ico: '⚠️', title: 'Flujo Crítico', txt: 'Gastas más de lo que ganas.' });
      }

      if (cuotasMesTotal > ingresosMesTotal * 0.4 && ingresosMesTotal > 0) {
        scr -= 25;
        desgloseArr.push({ text: 'Deudas > 40%', pts: -25, type: 'danger' });
      }
    ...
    const cuentasDisponiblesWizard = useMemo(() => {
      if (!qeMethod) return [];

      if (qeType === 'ingreso') {
        if (qeMethod === 'cash') return activeCalculatedAccounts.filter(c => c.type === 'cash');
        if (qeMethod === 'bank') return activeCalculatedAccounts.filter(c => ['bank', 'pocket', 'investment'].includes(c.type));
        return [];
      }

      return activeCalculatedAccounts.filter(c => c.type === qeMethod);
    }, [qeMethod, qeType, activeCalculatedAccounts]);

    const handleQuickSave = () => {
      if (!qeMonto || !qeCategoria || !qeCuenta) return;

      const today = getLocalToday();
      const montoNum = safeNumber(qeMonto);
      ...
      if (qeType === 'egreso') {
        ...
        addIngreso({
          ...
          monto: montoNum,
          cuentaId: qeCuenta,
          tipo: qeCategoria === 'Inversión' ? 'Rendimiento' : 'Variable'
        });
    ...
    const searchResults = useMemo(() => {
      ...
      const results = [
        ...activeEgresos
          .filter(e => String(e.descripcion || '').toLowerCase().includes(query) || String(e.categoria || '').toLowerCase().includes(query))
          .map(e => formatResult(e, 'Egreso', '📉', 'text-neonmagenta')),

        ...activeIngresos
          .filter(i => String(i.descripcion || '').toLowerCase().includes(query) || String(i.categoria || '').toLowerCase().includes(query))
          .map(i => formatResult(i, 'Ingreso', '📈', 'text-emerald-400')),

        ...activeTransferencias
          .filter(t => String(t.descripcion || '').toLowerCase().includes(query))
          .map(t => formatResult(t, 'Transferencia', '🔄', 'text-amber-400')),

        ...activeCalculatedAccounts
          .filter(c => String(c.name || '').toLowerCase().includes(query))
          .map(c => formatResult(c, 'Cuenta', '🏦', 'text-neoncyan'))
      ];

      return results.sort((a, b) => {
        const da = new Date(b.fecha === 'Sin fecha' ? 0 : b.fecha).getTime() || 0;
        const db = new Date(a.fecha === 'Sin fecha' ? 0 : a.fecha).getTime() || 0;
        return da - db;
      });
        ...
        <div className="min-h-screen bg-appbg text-slate-200 flex flex-col md:flex-row font-sans md:pt-0 pt-[24px] relative">
          ...
          <aside className="hidden md:flex w-64 bg-appcard flex-shrink-0 flex-col z-20 shadow-[4px_0_24px_rgba(0,0,0,0.3)]">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-800">
              <div className="text-[10px] font-black text-[#8A92A6] uppercase px-4 mb-3 tracking-widest mt-2">
                Diario
              </div>
              ...
              <div className="text-[10px] font-black text-[#8A92A6] uppercase px-4 mt-8 mb-3 tracking-widest">
                Estrategia
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl text-neonmagenta hover:shadow-glow-magenta hover:-translate-y-0.5 transition-all mt-6 border border-neonmagenta/30 font-bold text-sm bg-[#111222]"
              >
                Cerrar Sesión
              </button>
            </nav>
          ...
          <main className="flex-1 flex flex-col h-screen overflow-hidden pb-[72px] md:pb-0 relative">
            <div className="bg-appcard border-b border-white/[0.02] p-3 md:p-4 flex justify-between items-center gap-4 z-10">
              <button
                onClick={() => auth.signOut()}
                className="md:hidden text-neonmagenta p-2 hover:shadow-glow-magenta rounded-full transition-all"
              >
              ...
              {activeTab === 'deudas' && (
                <div className="flex bg-[#111222] shadow-neumorph-inset rounded-xl p-1 w-full md:w-auto border border-transparent">
                  ...
                  <button
                    onClick={() => setFiltroPersona('Andre')}
                    className={`flex-1 md:px-6 py-2 rounded-lg text-xs font-bold transition-all ${
                      filtroPersona === 'Andre'
                        ? 'bg-neonmagenta text-white shadow-glow-magenta'
                        : 'text-slate-500 hover:text-slate-300'
                    }`}
                  >
                    ANDRE
              ...
              <div className="flex-1 flex justify-end w-full md:w-auto gap-3">
                <button
                  ...
                  className="flex items-center justify-center w-[46px] h-[46px] bg-[#111222] shadow-neumorph-inset border border-transparent hover:border-neoncyan/30 rounded-xl text-slate-500 hover:text-neoncyan transition-all"
                  title={privacyMode ? 'Mostrar saldos' : 'Ocultar saldos'}
                >
                ...
                <div className="flex items-center bg-[#111222] shadow-neumorph-inset rounded-xl p-1 w-full md:max-w-[240px] justify-between">
                  <button onClick={() => changeMonth(-1)} className="p-2 text-slate-500 hover:text-neoncyan transition-colors">
                    <ChevronLeft size={18} />
                  </button>

                  <span className="font-bold text-white capitalize text-sm tracking-wide">
            ...
            {isOffline && (
              <div className="bg-rose-500/10 border-b border-rose-500/30 px-4 py-2 flex items-center gap-2 text-rose-400 text-xs font-bold">
                <AlertCircle size={14} />
                Estás sin conexión. Se usará la persistencia local de Firebase si está disponible.
              </div>
            )}
            ...
            {isHistoricalMonth && (
              <div className="bg-amber-500/10 border-b border-amber-500/30 px-4 py-2 flex items-center gap-2 text-amber-400 text-xs font-bold shadow-[0_0_15px_rgba(245,158,11,0.1)]">
                ...
                <button
                  ...
                  className="ml-auto bg-[#111222] border border-amber-500/30 hover:shadow-glow-amber px-3 py-1 rounded-lg text-amber-300 transition-all font-bold"
                >
                  Ir al actual →
                </button>
              </div>
                ...
                <ErrorBoundary>
                  {activeTab === 'dashboard' && (
                    <DashboardTab
                      ...
                      cuotasMesTotal={cuotasMesTotal}
                      cuotasMesRestantes={cuotasMesRestantes}
                      ingresosMesTotal={ingresosMesTotal}
                      egresosMesTotal={egresosMesTotal}
                      deudaTotal={deudaTotal}
                      liquidezTotal={liquidezTotal}
                      inversionTotal={inversionTotal}
                      selectedMonth={selectedMonth}
                      egresosMes={egresosMes}
                      ingresos={activeIngresos}
                      egresos={activeEgresos}
                      presupuestos={activePresupuestos}
                      pagosFijos={activePagosFijos}
                      ingresosFijos={activeIngresosFijos}
                  ...
                  {activeTab === 'cuentas' && (
                    <CuentasTab
                      ...
                      transferencias={activeTransferencias}
                      addTransferencia={addTransferencia}
                      removeTransferencia={removeTransferencia}
                      addEgreso={addEgreso}
                      showToast={showToast}
                      privacyMode={privacyMode}
                    />
                  ...
                  {activeTab === 'ingresos' && (
                    <IngresosTab
                      ...
                      removeIngreso={removeIngreso}
                      ingresosFijos={ingresosFijos}
                      addIngresoFijo={addIngresoFijo}
                      updateIngresoFijo={updateIngresoFijo}
                      removeIngresoFijo={removeIngresoFijo}
                      cuentas={calculatedAccounts}
                      selectedMonth={selectedMonth}
                      showToast={showToast}
                      privacyMode={privacyMode}
                    />
                  ...
                  {activeTab === 'egresos' && (
                    <EgresosTab
                      ...
                      removeEgreso={removeEgreso}
                      pagosFijos={activePagosFijos}
                      addPagoFijo={addPagoFijo}
                      updatePagoFijo={updatePagoFijo}
                      removePagoFijo={removePagoFijo}
                      comprasCuotas={activeComprasCuotas}
                      addComprasCuotas={addComprasCuotas}
                      removeComprasCuotas={removeComprasCuotas}
                      cuentas={activeCalculatedAccounts}
                      updateCuenta={updateCuenta}
                      removeCuenta={removeCuenta}
                      selectedMonth={selectedMonth}
                      presupuestos={activePresupuestos}
                      categoriasMaestras={categoriasMaestras}
                      showToast={showToast}
                      privacyMode={privacyMode}
                    />
                  )}
                  ...
                  {activeTab === 'presupuestos' && (
                    <PresupuestosTab
                      ...
                      removePresupuesto={removePresupuesto}
                      pagosFijos={activePagosFijos}
                      addPagoFijo={addPagoFijo}
                      updatePagoFijo={updatePagoFijo}
                      removePagoFijo={removePagoFijo}
                  ...
                  {activeTab === 'simulador' && (
                    <SimuladorTab
                      cuentas={activeCalculatedAccounts}
                      addPagoFijo={addPagoFijoToState}
                      showToast={showToast}
                      privacyMode={privacyMode}
                    />
                  )}
            ...
            <div className="flex px-2 min-w-max w-full">
              {navItems.map(item => (
                <button
                  ...
                  className="w-[76px] flex flex-col items-center justify-center p-2 relative group"
                >
                  {activeTab === item.id && (
                    <div className="absolute top-0 w-8 h-1 bg-neoncyan rounded-b-full shadow-glow-cyan"></div>
                  )}
                  ...
                  <item.icon
                    ...
                    className={`transition-all duration-300 ${
                      activeTab === item.id
                        ? 'mb-1 text-neoncyan'
                        : 'mb-1 text-slate-500 group-hover:text-slate-300'
                    }`}
                  />

                  <span className={`text-[9px] font-bold tracking-wide truncate w-full text-center transition-colors ${
          ...
          <button
            ...
            className="fixed bottom-[90px] md:bottom-8 right-4 md:right-8 w-14 h-14 bg-neoncyan text-[#0b0c16] rounded-full shadow-glow-cyan flex items-center justify-center z-40 transition-all hover:scale-110 active:scale-95"
          >
            <Plus size={28} />
          </button>
              ...
              <div className="bg-appcard w-full md:w-[420px] md:rounded-[30px] rounded-t-[30px] p-6 border border-white/[0.05] shadow-[0_20px_60px_rgba(0,0,0,0.6)] animate-in slide-in-from-bottom-10 min-h-[420px] flex flex-col relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-6 shrink-0 relative z-10">
                        ...
                        {[1, 2, 3, 4, 5].map(s => (
                          <div
                            ...
                            className={`h-1.5 rounded-full transition-all duration-500 ${
                              s === qeStep
                                ? 'bg-neoncyan w-6 shadow-glow-cyan'
                                : s < qeStep
                                  ? 'bg-neoncyan/40 w-3'
                                  : 'bg-[#111222] w-3'
                            }`}
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-rose-400 bg-[#111222] p-2.5 rounded-full transition-all hover:shadow-glow-magenta"
                  >
                    <XIconGlobal size={18} />
                  </button>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center relative z-10 animate-in fade-in slide-in-from-right-4 duration-300">
                  {qeStep === 1 && (
                    <div className="space-y-4">
                      <h4 className="text-center text-[#8A92A6] font-black uppercase tracking-widest text-xs mb-6">
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-3 p-5 bg-[#111222] border border-rose-500/20 hover:border-rose-500 hover:shadow-glow-magenta rounded-2xl text-rose-400 font-black text-lg transition-all hover:-translate-y-1"
                      >
                        📉 Registrar Gasto
                      </button>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-3 p-5 bg-[#111222] border border-emerald-500/20 hover:border-emerald-500 hover:shadow-[0_0_15px_rgba(16,185,129,0.4)] rounded-2xl text-emerald-400 font-black text-lg transition-all hover:-translate-y-1"
                      >
                        📈 Registrar Ingreso
                      </button>
                    </div>
                  ...
                  {qeStep === 2 && (
                    <div className="space-y-6">
                      <div>
                        <label className={`text-[10px] font-black uppercase tracking-widest block mb-2 ${qeType === 'egreso' ? 'text-rose-500' : 'text-emerald-500'}`}>
                          Monto exacto
                        </label>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`w-full bg-[#111222] shadow-neumorph-inset border border-transparent ${
                              qeType === 'egreso'
                                ? 'focus:border-rose-500 focus:shadow-glow-magenta text-rose-400'
                                : 'focus:border-emerald-500 focus:shadow-[0_0_15px_rgba(16,185,129,0.4)] text-emerald-400'
                            } rounded-2xl pl-10 pr-4 py-5 text-3xl font-black outline-none transition-all placeholder:text-slate-700`}
                            placeholder="0"
                            autoFocus
                          />
                        </div>
                      ...
                      {qeType === 'ingreso' && (
                        <div>
                          <label className="text-[10px] font-black text-[#8A92A6] uppercase tracking-widest block mb-2">
                          ...
                          <input
                            ...
                            className="w-full bg-[#111222] shadow-neumorph-inset border border-transparent focus:border-neoncyan text-white rounded-xl px-4 py-3.5 text-sm font-medium outline-none transition-all placeholder:text-slate-600"
                            placeholder="Ej: Pago quincena, rendimiento..."
                          />
                        </div>
                      )}
                      ...
                      <button
                        disabled={!qeMonto}
                        onClick={() => setQeStep(3)}
                        className="w-full py-4 rounded-xl font-black text-[#0b0c16] text-lg bg-neoncyan shadow-glow-cyan disabled:opacity-20 disabled:shadow-none transition-all"
                      >
                        Siguiente Paso
                      </button>
                    </div>
                  ...
                  {qeStep === 3 && (
                    <div className="h-full flex flex-col">
                      <h4 className="text-center text-[#8A92A6] font-black uppercase tracking-widest text-xs mb-4">
                        Selecciona la Categoría Macro
                      </h4>

                      <div className="flex-1 overflow-y-auto pr-2 pb-4 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-700">
                        <div className="grid grid-cols-2 gap-2.5">
                          {(qeType === 'egreso'
                            ? Object.keys(CATEGORIAS_CONFIG)
                            : ['Salario', 'Honorarios', 'Transferencia', 'Inversión', 'Regalo', 'Otros']
                          ).map(cat => (
                            <button
                              key={cat}
                              onClick={() => {
                      ...
                      <div className="flex-1 overflow-y-auto pr-2 pb-4 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-700">
                        <div className="grid grid-cols-1 gap-2.5">
                          {opcionesEspecificasWizard.map(opt => (
                            ...
                            >
                              {opt.específico}
                              <span className="block text-[9px] text-slate-500 mt-1 uppercase tracking-widest">
                                ↳ {opt.sub}
                              </span>
                            </button>
                          ))}
                  ...
                  {qeStep === 4 && (
                    <div className="space-y-4">
                      <h4 className="text-center text-[#8A92A6] font-black uppercase tracking-widest text-xs mb-6">
                        {qeType === 'egreso' ? '¿Cómo lo pagaste?' : '¿A dónde entró el dinero?'}
                      </h4>
                      ...
                      <button
                        ...
                        className="w-full p-4 rounded-xl text-sm font-bold text-left transition-all border border-white/[0.02] flex items-center gap-3 bg-[#111222] text-slate-300 hover:border-neoncyan hover:shadow-glow-cyan"
                      >
                      ...
                      <button
                        ...
                        className="w-full p-4 rounded-xl text-sm font-bold text-left transition-all border border-white/[0.02] flex items-center gap-3 bg-[#111222] text-slate-300 hover:border-neoncyan hover:shadow-glow-cyan"
                      >
                        <span className="text-xl">🏦</span> Cuenta / Ahorro / Inversión
                      </button>
                  ...
                  {qeStep === 5 && (
                    <div className="space-y-6">
                      <h4 className="text-center text-[#8A92A6] font-black uppercase tracking-widest text-xs mb-2">
                        {qeType === 'egreso' ? '¿De cuál cuenta exactamente?' : '¿A qué cuenta exactamente?'}
                      </h4>
                      ...
                      <div className="grid grid-cols-1 gap-2.5 overflow-y-auto max-h-[250px] pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-700">
                        {cuentasDisponiblesWizard.map(acc => (
                          <button
                            key={acc.id}
                            onClick={() => setQeCuenta(acc.id)}
                            className={`p-4 rounded-xl text-sm font-bold text-left transition-all border flex justify-between items-center ${
                              qeCuenta === acc.id
                                ? 'bg-neoncyan/10 border-neoncyan text-neoncyan shadow-glow-cyan'
                                : 'bg-[#111222] border-white/[0.02] text-slate-300 hover:border-slate-600'
                            }`}
                          >
                            <span>{acc.name}</span>
                            {qeCuenta === acc.id && (
                              <div className="w-2.5 h-2.5 bg-neoncyan rounded-full shadow-[0_0_8px_#00E5FF]"></div>
                            )}
                          </button>
                        ))}

                        {cuentasDisponiblesWizard.length === 0 && (
                          <div className="text-center p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl">
                            <p className="text-rose-400 text-xs font-bold">
                              No tienes cuentas de este tipo registradas.
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      {qeCuenta && (
                        <button
                          ...
                          className={`w-full py-4 rounded-xl font-black text-lg transition-all hover:scale-[1.02] active:scale-95 ${
                            qeType === 'egreso'
                              ? 'bg-rose-500 text-white shadow-[0_0_20px_rgba(244,63,94,0.4)]'
                              : 'bg-emerald-500 text-[#0b0c16] shadow-[0_0_20px_rgba(16,185,129,0.4)]'
                          }`}
                        >
                          ¡Confirmar y Guardar!
                        </button>
            ...
            <div className="fixed inset-0 bg-[#0b0c16]/90 backdrop-blur-md z-[60] flex flex-col items-center pt-10 md:pt-20 px-4 animate-in fade-in duration-200">
              <div className="w-full max-w-2xl flex flex-col relative animate-in slide-in-from-top-4">
                <div className="relative z-10">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 text-neoncyan">
                    <SearchIcon size={24} />
                  </span>
                  ...
                  <input
                    ...
                    placeholder="Busca un gasto, ingreso, cuenta o transferencia..."
                  ...
                  <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-rose-400 p-2 transition-colors"
                  >
                    <XIconGlobal size={20} />
                  </button>
                </div>
                    ...
                    {searchResults.length > 0 ? (
                      <div className="flex flex-col gap-1">
                        {searchResults.map((res, i) => (
                          <div
                            key={`${res.tipo}-${res.id}-${i}`}
                            className="flex justify-between items-center p-4 hover:bg-white/[0.02] rounded-xl transition-colors cursor-default border border-transparent hover:border-white/[0.05]"
                          >
                      ...
                      <div className="p-10 text-center">
                        <p className="text-[#8A92A6] text-sm font-bold tracking-wide">
                          No se encontraron resultados para "<span className="text-white">{searchQuery}</span>"
                        </p>
                      </div>
                    )}
                  </div>
          ...
          {/* UNDO */}
          {toast && toast.undoData && undoItem && (
            <button
              onClick={handleUndo}
              className="fixed bottom-40 md:bottom-28 right-4 md:right-10 bg-[#111222] border border-neoncyan/40 text-neoncyan px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-glow-cyan z-[101]"
            >
              Deshacer
            </button>
          )}
