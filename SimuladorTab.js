const SimuladorTab = ({ cuentas, addPagoFijo, showToast }) => {
  ...
  const calcSim1 = () => {
    ...
    const tm = getTasaMensual(s1Tasa); 
    const intMesActual = s1Saldo * tm;
    ...
    const simular = (pagoMensual) => { 
      // 🚨 FRENO 1: Si el pago es menor al interés, es una deuda infinita. Bloqueamos el cálculo.
      if (pagoMensual <= intMesActual) {
        return { meses: 600, intTotal: 0 };
      }

      let saldoRemanente = s1Saldo; 
      let meses = 0; 
      let intTotal = 0; 
      
      // 🚨 FRENO 2: El saldo debe ser mayor a 10 centavos para evitar loops infinitos por decimales.
      while(saldoRemanente > 0.1 && meses < 600) { 
    ...
    return { 
      mesesNormal: escenarioNormal.meses, 
      intNormal: Math.round(escenarioNormal.intTotal), 
      mesesAcelerado: escenarioAcelerado.meses, 
      intAcelerado: Math.round(escenarioAcelerado.intTotal), 
