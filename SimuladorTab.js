...
const SelectUI = ({ label, options, value, onChange, className }) => (
...
const InputUI = ({ type="text", label, value, onChange, className, placeholder, disabled=false }) => (
...
const CardUI = ({ children, className }) => (
...
const SimuladorTab = ({ cuentas, addPagoFijo, showToast, privacyMode }) => {
  ...
  // ============================================================================
  // CÁLCULOS DEL SIMULADOR 1: PAGO ACELERADO
  // ============================================================================
  const calcSim1 = () => {
  ...
  const asumirAcelerado = () => {
    ...
    addPagoFijo({ id: generateId(), descripcion: `Abono Acelerado: ${dSeleccionada.name}`, categoria: 'Deudas', monto: s1Cuota + s1.extra, diaPago: 1 });
    showToast(`¡Simulación guardada! Abono de ${formatCOP(s1Cuota + s1.extra)} en Pagos Fijos.`);
  };

  // ============================================================================
  // CÁLCULOS DEL SIMULADOR 2: PLAN DE AHORRO
  // ============================================================================
  ...
  const asumirAhorro = () => {
    ...
    addPagoFijo({ id: generateId(), descripcion: `Ahorro: ${s2.nombre}`, categoria: 'Inversión', monto: resS2.aporteMensual, diaPago: 1 });
    showToast(`Plan activado. Cuota de ${formatCOP(resS2.aporteMensual)} en Pagos Fijos.`);
  };

  // ============================================================================
  // CÁLCULOS DEL SIMULADOR 3: INTERÉS COMPUESTO
  // ============================================================================
  ...
  const asumirInversion = () => {
    ...
    addPagoFijo({ id: generateId(), descripcion: `Aporte a Inversión: ${nombreDesc}`, categoria: 'Inversión', monto: s3.aporte, diaPago: 1 });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 md:pb-0 max-w-4xl mx-auto">
      ...
      {activeSim === 'acelerado' && (
        <CardUI className="!border-transparent animate-in slide-in-from-bottom-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            <SelectUI label="Elige tu crédito o tarjeta" options={cuentas.filter(c=>['credit','loan'].includes(c.type)).map(c=>({value:c.id,label:`${c.name} (Deuda: ${formatCOP(c.currentDebt)})`}))} value={s1.deudaId} onChange={e=>setS1({...s1, deudaId: e.target.value})} className="sm:col-span-2" />
            <InputUI type="number" label="Abono EXTRA mensual ($)" value={s1.extra} onChange={e=>setS1({...s1, extra: Number(e.target.value)})} className="sm:col-span-2 font-black text-neonmagenta" />
          </div>
      ...
      {activeSim === 'ahorro' && (
        <CardUI className="!border-transparent animate-in slide-in-from-bottom-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            <InputUI label="Nombre del Plan (Ej: Viaje, Carro)" placeholder="Escribe tu meta..." value={s2.nombre} onChange={e=>setS2({...s2, nombre: e.target.value})} className="sm:col-span-2 font-bold text-white" />
            <InputUI type="number" label="¿Cuánto dinero necesitas? ($)" value={s2.objetivo} onChange={e=>setS2({...s2, objetivo: Number(e.target.value)})} className="font-black text-neoncyan" />
            <InputUI type="number" label="¿En cuántos meses? (Ej: 12)" value={s2.meses} onChange={e=>setS2({...s2, meses: Number(e.target.value)})} className="font-bold text-white" />
            <InputUI type="number" label="Rendimiento del banco E.A. (%)" value={s2.tasaEA} onChange={e=>setS2({...s2, tasaEA: Number(e.target.value)})} className="font-bold text-amber-400" />
            <InputUI type="number" label="Dinero ya ahorrado ($)" value={s2.ahorrado} onChange={e=>setS2({...s2, ahorrado: Number(e.target.value)})} className="font-bold text-white" />
          </div>
      ...
      {activeSim === 'inversion' && (
        <CardUI className="!border-transparent animate-in slide-in-from-bottom-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            <SelectUI label="Seleccionar Inversión" className="sm:col-span-2" 
            ...
            <InputUI type="number" label="Capital Inicial ($)" value={s3.capital} onChange={e=>setS3({...s3, capital: Number(e.target.value)})} disabled={s3.invId !== 'otra'} className="font-bold text-white" />
            <InputUI type="number" label="Tasa E.A. (%)" value={s3.tasa} onChange={e=>setS3({...s3, tasa: Number(e.target.value)})} disabled={s3.invId !== 'otra'} className="font-bold text-amber-400" />
            <InputUI type="number" label="Tiempo (Años)" value={s3.anos} onChange={e=>setS3({...s3, anos: Number(e.target.value)})} className="font-bold text-white" />
            <InputUI type="number" label="Aporte Adicional Mensual ($)" value={s3.aporte} onChange={e=>setS3({...s3, aporte: Number(e.target.value)})} className="!border-amber-500/30 focus:!shadow-glow-amber font-black text-amber-400" />
          </div>
